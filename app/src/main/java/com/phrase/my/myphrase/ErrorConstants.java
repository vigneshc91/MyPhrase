package com.phrase.my.myphrase;

/**
 * Created by vignesh on 7/3/17.
 */

public class ErrorConstants {

    public static final String ERROR = "Error";
    public static final String ALERT = "Alert";
    public static final String REQUIRED_FIELDS_EMPTY = "Required Fields Empty.";

    public static final String PASSWORD_ADD_FAILED = "Password Add Failed";
    public static final String PASSWORD_EDIT_FAILED = "Password Edit Failed";
    public static final String INVALID_MPIN = "Invalid mpin, Please try again";
    public static final String WRONG_MPIN = "Wrong mpin, Please try again";
    public static final String PASSWORD_DELETE_FAILED = "Password delete failed";
    public static final String WANT_TO_DELETE_PASSWORD = "Are you sure? Do you want to delete this password.";
    public static final String WANT_TO_DELETE_PASSWORDS = "Are you sure? Do you want to delete the selected passwords.";

}
